...
- The repo is a pnpm workspace (packageManager pinned at 10.11.0) with Docker-backed PostgreSQL; `pnpm run dev` boots both the API (http://localhost:5050) and web app (http://localhost:3030).
- Backend code uses TypeScript + Hono + oRPC on top of PostgreSQL via Drizzle ORM; shared contracts live in packages/shared and drive both REST/OpenAPI and RPC layers.
...
- Follow the workspace workflow: open an issue, branch as `<issue>-<slug>`, commit with `git cz` (or `pnpm commit`), keep Docker running for PostgreSQL, and avoid rebases on main.
...
- `apps/server`: API entry (`src/index.ts`), oRPC context/auth (`src/lib`), routers, loaders, and Drizzle schemas (`src/db/schema`).
...
- `postgres-data`: Local PostgreSQL volume used by docker-compose.dev.yml (avoid committing changes here).

...
function DropdownMenuContent({
  ...
  sideOffset = 4,
  ...props
}: DropdownMenuPrimitive.Popup.Props & Pick<DropdownMenuPrimitive.Positioner.Props, 'align' | 'side' | 'sideOffset'>) {
  return (
...
function DropdownMenuItem({
  ...
  variant = 'default',
  ...props
}: DropdownMenuPrimitive.Item.Props & { inset?: boolean; variant?: 'default' | 'destructive' }) {
  return (
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuPrimitive.SubmenuTrigger
      ...
      data-inset={inset}
      className={cn(
        'flex cursor-default items-center rounded-sm px-2 py-1.5 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-inset:pl-8 data-open:bg-accent data-open:text-accent-foreground',
        className,
      )}
      {...props}
    >
...
function DropdownMenuSubContent({ className, ...props }: ComponentProps<typeof DropdownMenuContent>) {
  return (
    <DropdownMenuContent
